...
class Response {
  ...
  Response(this.id_question, this.id_response, this.title, this.score);
}
      ...
      await _firestore.collection('questionnaire').document('response').collection('competency').document(list[index].faculty).collection(widget.id).where('id_answer', isEqualTo: '${_listQuestionCompetency[i].id_question}/${list[index].id}').getDocuments().then((answer){
        if(answer.documents.isNotEmpty){
          answer.documents.forEach((f) {
            Response response = new Response(_listQuestionCompetency[i].id_question, f.documentID, _listQuestionCompetency[i].title, f.data['score']);
  ...
  _getAchievementResponseFromCloudFirestore(List<User> list, int index) async {
    ...
    for(int i = 0; i < _listQuestionAchievement.length; i++){
      await _firestore.collection('questionnaire').document('response').collection('achievement').document(list[index].faculty).collection(widget.id).where('id_answer', isEqualTo: '${_listQuestionAchievement[i].id_question}/${list[index].id}').getDocuments().then((answer){
        if(answer.documents.isNotEmpty){
          answer.documents.forEach((f) {
            Response response = new Response(_listQuestionAchievement[i].id_question, f.documentID, _listQuestionAchievement[i].title, f.data['score']);
            _listTemp.add(response);
          });
        }
      }).then((value){
      ...
      pdf.addPage(pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Table.fromTextArray(
            context: context, 
            data: <List<String>>[
              <String>['No', 'Pertanyaan', 'Jawaban'],
              for(int i = 0; i < _listLearningExperience.length; i++)
              <String>['${i+1}', '${_listLearningExperience[i].title}', '${_listLearningExperience[i].score}'],
            ]
          ),
          pw.Padding(
            padding: pw.EdgeInsets.only(top: 20.0, bottom: 10.0),
          ...
          pw.Table.fromTextArray(
            ...
            data: <List<String>>[
              ...
              <String>['${i+1}', '${_listAchievementCompetence[i].title}', '${_listAchievementCompetence[i].score}'],
            ]
          ),
        ]
      ));
      // final directory = (await getExternalStorageDirectory()).path;
      final directory = new Directory('/storage/emulated/0/Tracer Studi UPN/$_level_departement ${widget.name}');
      if((await directory.exists())){
        print('SUDAH ADA');
                    ...
                    child: Material(
                      ...
                      child: IconButton(
                        ...
                        onPressed: (){
                          ...
                          Share.shareFiles(_listPath, text: 'Laporan $_level_departement ${widget.name} Tracer Studi UPN');
                        },
                      ),
                    ),
                  )
          ...
          return Material(
            ...
            child: InkWell(
              ...
              onLongPress: (){
                ...
                // Share.shareFiles([pdf.path], text: 'Laporan $_level_departement ${widget.name} Tracer Studi UPN');
              },
              borderRadius: BorderRadius.circular(15.0),
              child: Container(
                width: double.infinity,
