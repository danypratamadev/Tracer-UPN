...
class StaticAchievementPage extends StatefulWidget {

  final String id, name;

  const StaticAchievementPage({Key key, @required this.id, @required this.name}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return StaticAchievementState();
  }
...
class StaticAchievementState extends State<StaticAchievementPage> {
  ...
  _getResponseQuestionFromCloudFirestore() async {
    for(int i = 0; i < _listQuestion.length; i++){
      ...
      _firestore.collection('questionnaire').document('response').collection('achievement').document(_id_faculty).collection(widget.id).where('id_question', isEqualTo: _listQuestion[i].id).getDocuments().then((value){
        if(value.documents.isNotEmpty){
          ...
          Response response = new Response('Tidak Baik', one);
          Response response2 = new Response('Baik', two);
          Response response3 = new Response('Sangat Baik', three);
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        'Berikut daftar statik capaian kompetensi untuk jurusan ${widget.name}',
                        maxLines: 3,
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: Align(
                            ...
                            child: FlatButton(
                              onPressed: (){
                                Navigator.push(context, MaterialPageRoute(builder: (context) => StaticDetailsPage(action: 20, id_soal: _listQuestion[index].id, title: _listQuestion[index].title, number: index, id: widget.id, name: widget.name,)));
                              }, 
                              child: Text(
                                'Lihat Detail'
                              ),
