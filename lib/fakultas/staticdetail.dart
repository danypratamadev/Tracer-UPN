...
class StaticDetailsPage extends StatefulWidget {
  ...
  final String id, id_soal, name, title;
  final int action, number;

  const StaticDetailsPage({Key key, @required this.action, @required this.id, @required this.title, @required this.number, @required this.id_soal, @required this.name}) : super(key: key);
...
class StaticDetailsState extends State<StaticDetailsPage> {
  ...
  _getResponseQuestionFromCloudFirestore() async {
    ...
    await _firestore.collection('questionnaire').document('response').collection(category).document(_id_faculty).collection(widget.id).where('id_question', isEqualTo: widget.id_soal).getDocuments().then((value){
    ...
    for(int i = 0; i < _listTemp.length; i++){
      _firestore.collection('user').document(_listTemp[i].id_user).collection('biodata').document('data').get().then((value){
        if(value.exists){
          ...
          _firestore.collection('departement').document(widget.id).collection('level').document(value.data['level']).get().then((level){
            if(level.exists){
