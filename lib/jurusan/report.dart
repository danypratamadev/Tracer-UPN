...
class ReportState extends State {
  ...
  double _height = 0.0;
  String _id_departement, _name_departement, _id_level, _level_departement;
  ...
  _getAllLevelDepartement() async {
    _firestore.collection('departement').document(_id_departement).collection('level').getDocuments().then((value){
      ...
      if(user.documents.isNotEmpty){
        ...
        user.documents.forEach((f) {
          _firestore.collection('user').document(f.documentID).collection('biodata').document('data').get().then((value){
            if(value.exists){
              if(value.data['departement'] == _id_departement && !f.data['delete'] && value.data['level'] == _id_level && value.data['competency'] && value.data['achievement']){
                String initial;
  ...
  _getCarrerResponseFromCloudFirestore(List<User> list, int index) async {
    ...
    await _firestore.collection('questionnaire').document('response').collection('carrer').document(list[index].faculty).collection(_id_departement).document(list[index].id).get().then((value){
      if(value.exists){
  ...
  _getSearchAlumnusResponseFromCloudFirestore(List<User> list, int index) async {
    ...
    if(mounted){
      ...
      for(int i = 0; i < _listTemp.length; i++){
        ...
        await _firestore.collection('questionnaire').document('response').collection('searchalumnus').document(list[index].faculty).collection(_id_departement).where('id_answer', isEqualTo: '${_listTemp[i].id_question}/${list[index].id}').getDocuments().then((answer){
  ...
  _getCompetencyResponseFromCloudFirestore(List<User> list, int index) async {
    ...
    for(int i = 0; i < _listQuestionCompetency.length; i++){
      await _firestore.collection('questionnaire').document('response').collection('competency').document(list[index].faculty).collection(_id_departement).where('id_answer', isEqualTo: '${_listQuestionCompetency[i].id_question}/${list[index].id}').getDocuments().then((answer){
        if(answer.documents.isNotEmpty){
          String grade;
          answer.documents.forEach((f) {
            if(f.data['score'] == 1){
  ...
  _getAchievementResponseFromCloudFirestore(List<User> list, int index) async {
    ...
    for(int i = 0; i < _listQuestionAchievement.length; i++){
      await _firestore.collection('questionnaire').document('response').collection('achievement').document(list[index].faculty).collection(_id_departement).where('id_answer', isEqualTo: '${_listQuestionAchievement[i].id_question}/${list[index].id}').getDocuments().then((answer){
        if(answer.documents.isNotEmpty){
          String grade;
          answer.documents.forEach((f) {
            if(f.data['score'] == 1){
  ...
  _exportToPdfFile(List<User> list, int index) async {
    if(await Permission.storage.request().isGranted){
      ...
      if((await directory.exists())){
        print('SUDAH ADA');
        final String path = '${directory.path}/${list[index].nim}_${list[index].name}.pdf';
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Text(
                              'Mengelola Laporan atau Membuat Laporan Baru pada Jurusan $_name_departement.',
                              maxLines: 3,
