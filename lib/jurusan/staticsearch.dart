...
class StaticSearchState extends State {
  ...
  _getResponseQuestionFromCloudFirestore() async {
    for(int i = 0; i < _listQuestion.length; i++){
      if(_listQuestion[i].type == 5){
        ...
        await _firestore.collection('questionnaire').document('response').collection('searchalumnus').document(_id_faculty).collection(_id_departement).where('id_question', isEqualTo: _listQuestion[i].id).getDocuments().then((value){
          if(value.documents.isNotEmpty){
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        'Berikut daftar statik penelusuran alumni untuk jurusan $_name_departement',
                        maxLines: 3,
